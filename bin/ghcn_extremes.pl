...
generated by ghcn_fetch -report id; i.e. it is a detail level report
of daily weather data by station id.

In typical use, you would extract the data of interest with

    ghcn_fetch -report id <other_options>
...
The option "-report id" can also be used, but if the data contains
